"""
LightGBM Model Training - 4h Interval

Walk-Forward Validation with Feature Selection for 4-hour trading strategy.
...
class LightGBMTrainer:
    """LightGBM Walk-Forward Validation for 4h interval"""
    
    def __init__(self, data_path: str = "datalake/4_diamond/4h.parquet"):
        self.data_path = Path(data_path)
        self.interval = "4h"
        
        # Walk-Forward Configuration for 4h
        self.test_size = 375            # ~2-2.5 months of 4h data
        self.min_train_size = 540       # ~3 months (minimum train)
        self.step_size = 84             # ~2 weeks step
        
        # Model Configuration (adjusted for smaller dataset)
        self.params = {
            'objective': 'binary',
            'metric': 'binary_logloss',
            'boosting_type': 'gbdt',
            'n_estimators': 600,        # Reduced for smaller dataset
            'learning_rate': 0.08,      # Higher learning rate
            'num_leaves': 23,           # Reduced complexity
        ...
        self.prediction_threshold = 0.55  # Relaxed confidence for 4h
    ...
    def load_data(self):
        ...
        print(f"⚠️  Warning: Small dataset - results may have high variance")
    ...
    def feature_selection(self, X_train, y_train, X_test):
        """Feature selection: Correlation filter + LightGBM importance"""
        ...
        print("🔧 Step 2: LightGBM importance selection...")
        ...
        fs_params.update({
            'n_estimators': 50,   # Even faster for small datasets
            'learning_rate': 0.15,
            'verbose': -1
        })
        ...
        # For small datasets, be more conservative with feature selection
        if self.feature_threshold == 'median':
            # Use top 60% instead of 50% for small datasets
            threshold_percentile = 40  # Keep top 60%
    ...
    def train_model(self, X_train, y_train, X_val=None, y_val=None):
        ...
        model = lgb.train(
            ...
            callbacks=[
                lgb.early_stopping(30),  # Reduced for smaller datasets
    ...
    def print_summary(self):
        ...
        print("="*60)
        print("⚠️  Note: Small dataset - statistical significance may be limited")
    ...
    def save_results(self):
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write("WARNING: Small dataset - statistical significance limited\\n\\n")
...
def main():
    parser = argparse.ArgumentParser(description=f'LightGBM Walk-Forward Validation for 4h interval')
    parser.add_argument('--data-path', default='datalake/4_diamond/4h.parquet', help='Path to training data')
    
    args = parser.parse_args()
    
    print(f"🚀 Starting LightGBM Training for 4h interval")
