...
class IndexPriceKlinesAggregator:
    """
    IndexPriceKlines aggregator using Pre-sort + Binary Search + Sliding Window
    """
    @staticmethod
...
def process_single_file(input_file: Path, output_dir: Path, symbol: str, all_files: list):
    """Process single indexPriceKlines file for all intervals"""
    ...
    try:
        ...
        for interval in intervals:
            try:
                aggregator = IndexPriceKlinesAggregator(interval=interval, step_size=step_size, max_workers=4)
                result_df = aggregator.run_pipeline(df_data, target_date=target_date)
                
                if result_df is not None and len(result_df) > 0:
                    output_dir_path = output_dir / "indexPriceKlines" / symbol / interval
...
def auto_process_indexpriceklines():
    """Auto discover and process all indexPriceKlines files in bronze"""
    
    # Auto-detect paths
    bronze_index_dir = Path("datalake/1_bronze/indexPriceKlines/btc-usdt/1m")
    silver_output_dir = Path("datalake/2_silver")
    symbol = "btc-usdt"
    
    print("🎯 AUTO PROCESSING INDEXPRICEKLINES FILES")
    print(f"📂 {bronze_index_dir}")
    print("=" * 50)
    
    # Check if input directory exists
    if not bronze_index_dir.exists():
        print(f"❌ Bronze directory not found: {bronze_index_dir}")
        return False
    
    # Get all parquet files
    parquet_files = sorted(list(bronze_index_dir.glob("*.parquet")))
    
    if not parquet_files:
        print(f"❌ No parquet files found in {bronze_index_dir}")
        return False
...
if __name__ == "__main__":
    success = auto_process_indexpriceklines()
    
    if success:
        print("\n🏁 All indexPriceKlines files processed successfully!")
    else:
